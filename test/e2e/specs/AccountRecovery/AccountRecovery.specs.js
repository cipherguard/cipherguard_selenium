...
describe("password workspace", () => {
  const admin = "admin@cipherguard.com";
  ...
  it("When an account recovery is approved, notify the user.", async () => {
    await SeleniumPage.checkSubjectContent(admin, "Admin User (admin@cipherguard.com) has approved your recovery request.", templates.accountRecovery.AN.approved)
    await LoginPage.goToLogin();
    await LoginPage.login(admin);
    await DisplayMainMenuPage.switchAppIframe();
    await DisplayMainMenuPage.signOut();
